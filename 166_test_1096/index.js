...
const { replacePlaceholders, delay, connectToBrowser, getCurrentPage } = require('./utils_166.js');
...
const moduleFunction = (async function (element, savedObjects, connections, elementMap, currentElementId, uuid, port, wsEndpoint) {
    ...
    const delayTime = parseInt(element.settings.delay || 1000, 10);

    logger.info(`Ожидание ${delayTime} мс через функцию waitWithSetTimeout().`);
    await waitWithSetTimeout(delayTime);
