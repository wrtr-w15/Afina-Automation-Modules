...
const { replacePlaceholders, delay, connectToBrowser, getCurrentPage } = require('./utils_144.js');
...
const moduleFunction = (async function (element, savedObjects, connections, elementMap, currentElementId, uuid, port, wsEndpoint) {
  ...
  const xpathRaw = replacePlaceholders(element.settings.xpath,savedObjects);
  const xpath = `::-p-xpath(${xpathRaw})`;

    try {
        // Получаем все элементы, соответствующие XPath
        const elements = await page.$$(xpath);

        const count = elements.length;
        const indices = [...Array(count).keys()]; // [0, 1, 2, ...]

        logger.info(`Найдено элементов: ${count} по XPath: ${xpathRaw}`);

        savedObjects[element.settings.saveTo] = count;
        return savedObjects;
    } catch (error) {
        logger.error(`Ошибка при поиске по XPath: ${error.message}`);
        return false;
    }
});;
