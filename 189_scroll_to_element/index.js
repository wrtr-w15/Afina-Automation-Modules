...
const { replacePlaceholders, delay, connectToBrowser, getCurrentPage } = require('./utils_189.js');
...
const moduleFunction = (async function (element, savedObjects, connections, elementMap, currentElementId, uuid, port, wsEndpoint) {
  ...
  const rawXpath = `::-p-xpath(${replacePlaceholders(element.settings.xpathone , savedObjects)})`;
  logger.info(`XHATH : -> ${rawXpath}`);
  console.log(`XHATH : -> ${rawXpath}`);
